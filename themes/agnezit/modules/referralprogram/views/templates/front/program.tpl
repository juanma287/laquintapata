	...
	<div id="idTab1" class="tab-pane active">
		...
		{if $canSendInvitations}
			...
			<form method="post" action="{$link->getModuleLink('referralprogram', 'program', [], true)|escape:'html':'UTF-8'}" class="std">
				...
				<p class="submit">
                    <button type="submit" id="submitSponsorFriends" name="submitSponsorFriends" class="btn btn-default button button-medium">{l s='Validate' mod='referralprogram'}<i class="icon-chevron-right right"></i></button>
				</p>
			</form>
		{else}
...
<ul class="footer_links clearfix">
	<li>
		...
		<i class="icon-chevron-left"></i> {l s='Back to Your Account' mod='referralprogram'}</a>
	</li>
	<li><a class="btn btn-default button button-small" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}" title="{l s='Home' mod='referralprogram'}"><i class="icon-chevron-left"></i>{l s='Home' mod='referralprogram'}</a></li>
</ul>
