...
class Anscrolltop extends Module
{
    ...
    protected function generateCSS()
    {
        ...
        $cfg[self::PREFIX.self::OPACITY] = number_format((int)$cfg[self::PREFIX.self::OPACITY]/100, 2);
        $method = "position".str_replace("-", "", $cfg[self::PREFIX.self::POSITION]);
    ...
    protected function getParam($key, $value = null)
    {
        ...
        return is_null($value) ? Configuration::get(self::PREFIX.$key) : Configuration::updateValue(self::PREFIX.$key, $value);
    }
    ...
    protected function postProcess()
    {
        foreach ($this->getConfigKeys() as $key) {
            if (in_array($key, $this->getConfigColorKeys())) {
                if (!preg_match('/^(#[0-9a-fA-F]{6})$/', Tools::getValue($key))) {
                    $this->_errors[] = Tools::displayError($this->l("That is not a color"));
                    continue;
                }
            }

            $this->getParam($key, Tools::getValue($key));
        }
    ...
    public function hookBackOfficeHeader()
    {
        if (in_array($this->name, array(Tools::getValue('module_name'), Tools::getValue('configure')))) {
            $this->context->controller->addCss("http://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css");
    ...
    public function hookHeader()
    {
        if (version_compare(_PS_VERSION_, "1.7.0.0", "<")) {
            ...
            $this->context->controller->registerStylesheet("modules-scrolltop-fe1", "http://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css", ['priority' => 1, 'server' => 'remote']);
            $this->context->controller->registerStylesheet("modules-scrolltop-fe2", "modules/{$this->name}/views/css/".$this->getParam(self::CSS_FILE), ['priority' => 150]);
            $this->context->controller->registerJavascript("modules-scrolltop-fe3", "modules/{$this->name}/views/js/front.js", ['position' => AbstractAssetManager::DEFAULT_JS_POSITION, 'priority' => 150]);
        }
    }
